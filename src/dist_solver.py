...
class DistributionAwareSolver:
    ...
    def initialize_policy(
        ...
        A = (
            xs
            - np.asarray([self.rho[a] for a in self.attr_order], dtype=float)[None, :]
        )
        ...
        for t in range(1, T + 1):
            ...
            lam = np.minimum(
                self.lambda_max, np.maximum(0.0, lam - (alpha0 / math.sqrt(t)) * g)
            )
        s = 1.0 + A @ lam
        ...
        for _ in range(300):
            ...
            if np.all(sl >= -1e-10):
                break
            lam = np.minimum(
                self.lambda_max, np.maximum(0.0, lam + 0.5 * (-np.minimum(sl, 0.0)))
            )
            s = 1.0 + A @ lam
            a = (s > 0.0).astype(float)
    ...
    def _guard_must_attrs(
        self, current_counts: Dict[str, int], admitted: int
    ) -> List[str]:
        R = self.N - admitted
        must = []
    ...
    def _type_accept_prob(self, attributes: Dict[str, bool]) -> float:
        ...
        if i is None:
            diffs = np.sum(
                np.abs(self.xs - np.asarray(key, dtype=np.uint8)[None, :]), axis=1
            )
    ...
    def should_accept(
        ...
        helps_needed = any(
            attributes.get(a, False) and d[a] > 0 for a in self.attr_order
        )
        if R <= self.endgame_R:
